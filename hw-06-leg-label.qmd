...
**Comparing Models**\
Overall, the reasoning and detailed prompts perform significantly better than naive prompts across all models, highlighting the importance of prompt engineering in leveraging LLM capabilities effectively. Across models, GPT-5 consistently outperforms GPT-4.1 and GPT-5-nano, indicating that larger models with more advanced architectures yield better classification performance. Sensitivity remains a challenge, particularly for smaller models and naive prompts, suggesting that these configurations struggle to identify positive instances effectively. Specificity is generally high across all models and prompts, indicating that negative instances are classified correctly more often. These insights underscore the value of using larger LLMs with well-crafted prompts for classification tasks to achieve optimal accuracy and balance between sensitivity and specificity.

I was unable to retain token usage with chatlas. 
...
For this homework, I utilized generative AI tools for coding and help with designing reasoning prompts. I also used Generarive AI to help build my shiny app. I think I demonstrated mastery of topics by effectively using batch processing to evaluate all 9 model and prompt combinations, compare them using accuracy, f1, sensitivity, and specificity. I also created visualizations to help compare model performance. Lastly, I created a shiny app using RAG to store all the homeworks, syllabus and projects then added some extra functionality such as language/topic selections and tips/example questions.